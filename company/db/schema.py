...
class CompanyTag(Model, BaseMixin):
    ...
    tag = fields.ForeignKeyField(
        model_name="models.Tag", on_delete=fields.CASCADE, related_name="company_tags"
    )
...
class TagLanguage(Model, BaseMixin):
    tag = fields.ForeignKeyField(
        model_name="models.Tag", on_delete=fields.CASCADE, related_name="tag_languages"
    )
    language = fields.ForeignKeyField(
        model_name="models.Language",
        on_delete=fields.CASCADE,
        related_name="tag_languages",
    )
    name = fields.CharField(max_length=255)
